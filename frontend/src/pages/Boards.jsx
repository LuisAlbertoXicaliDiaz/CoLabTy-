...
export default function Boards() {
  ...
  if (!user || loading) {
    return <div className="flex h-screen items-center justify-center bg-slate-950 text-slate-400 font-sans">Cargando tableros...</div>;
  }
    ...
    <div className={`flex h-screen ${currentTheme.bg} font-sans overflow-hidden transition-colors duration-300`}>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <header className={`h-20 border-b ${currentTheme.header} backdrop-blur-xl flex items-center justify-between px-8 sticky top-0 z-20`}>
          <div>
            <h2 className={`text-xl font-bold tracking-tight ${currentTheme.textMain}`}>Gestión de Tableros</h2>
            <p className={`text-xs ${currentTheme.textMuted}`}>Administra tus proyectos Kanban y colaboradores.</p>
          </div>
        ...
        <div className="p-8 max-w-7xl mx-auto w-full space-y-8">
          <div className="flex justify-between items-center">
            <div>
              <h2 className={`text-2xl font-black ${currentTheme.textMain}`}>Tus Tableros Kanban</h2>
              <p className={`text-sm ${currentTheme.textMuted} mt-1`}>Visualiza y accede a tus tableros personales y compartidos.</p>
            </div>
            <button 
              onClick={() => setIsModalOpen(true)}
              className={`font-bold px-5 py-3 rounded-xl transition-all shadow-lg flex items-center gap-2 cursor-pointer ${currentTheme.buttonPrimary}`}
            >
          ...
          {loading ? (
            <p className={currentTheme.textMuted}>Cargando tableros...</p>
          ) : boards.length === 0 ? (
            <div className={`${currentTheme.card} border rounded-2xl p-12 text-center space-y-3`}>
              <p className={`${currentTheme.textMuted} font-medium`}>Aún no tienes ningún tablero creado o compartido.</p>
              <button onClick={() => setIsModalOpen(true)} className="text-indigo-400 font-bold hover:underline cursor-pointer">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {boards.map((board) => (
                <Link 
                  to={`/boards/${board.id}`}
                  key={board.id} 
                  className={`${currentTheme.card} border hover:border-indigo-500/40 p-6 rounded-2xl transition-all flex flex-col justify-between h-44 group shadow-sm relative border-l-4 border-l-indigo-600`}
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className={`text-lg font-bold ${currentTheme.textMain} group-hover:text-indigo-400 transition-colors pr-6`}>{board.title}</h3>
                      {board.user && (
                        <p className="text-xs text-indigo-400 font-semibold mt-1">Creador: {board.user.name}</p>
                      )}
                    </div>
                    {board.userId === user.id && (
                      <button 
                        onClick={(e) => handleDeleteBoard(e, board.id)}
                        className="text-slate-400 hover:text-red-500 font-bold p-1 rounded-lg hover:bg-slate-800/20 transition-colors z-10 cursor-pointer"
                  ...
                  <div>
                    <p className={`text-xs ${currentTheme.textMuted} mt-1`}>Creado el {new Date(board.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div className={`flex justify-between items-center text-xs ${currentTheme.textMuted} pt-4 border-t border-slate-700/20 font-medium`}>
                    ...
                    <span className="text-indigo-400 group-hover:translate-x-1 transition-transform font-semibold">Ver tablero →</span>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 text-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-6">
            <div className="flex justify-between items-center border-b border-slate-800 pb-4">
              <h3 className="text-xl font-bold">Crear Nuevo Tablero</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-white font-bold text-xl cursor-pointer">✕</button>
            </div>

            <form onSubmit={handleCreateBoard} className="space-y-4">
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 block mb-2">Nombre del Proyecto / Tablero</label>
                <input 
                  type="text" 
                  value={newBoardTitle}
